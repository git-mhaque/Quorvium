...
This directory contains Terraform configuration that provisions the baseline cloud footprint for Quorvium:
...
- Firestore API enablement and optional Firestore Native database provisioning for persistent board/note storage.
...
2. Create environment-specific Artifact Registry repositories referenced by `cloud_run_image` (each repository is created once per project):
   ...
   ```
3. Copy `terraform.tfvars.example` to `terraform.tfvars` and fill in project-specific values.
   - `cloud_run_image` is only a bootstrap image used when Terraform creates the service for the first time and is optional (Terraform defaults it to a public Cloud Run hello image).
   - Set `enable_firestore_database=true` for staging if Terraform should provision Firestore.
   - Set `firestore_location` to the staging region (for example `australia-southeast1`).
   - Keep `firestore_database_name="(default)"` unless you are intentionally using non-default databases.
   Minimal staging example:
   ```hcl
   project_id                 = "quorvium"
...
- Runtime SA receives `roles/datastore.user` so the API can read/write Firestore when `DATA_STORE=firestore`.
- If `enable_firestore_database=true` and a Firestore database already exists, Terraform create will fail. Import it into state:
  ```sh
  terraform import \
    'google_firestore_database.default[0]' \
    'projects/quorvium/databases/(default)'
  ```
- `DATA_DIR` defaults to `/tmp/quorvium-data` on Cloud Run and is only relevant when `DATA_STORE=file` (ephemeral mode).
