# OAuth Secret Operations

The current deployment footprint keeps the Quorvium API on Cloud Run with file-backed board storage for limited testing. This document focuses on how to manage Google OAuth credentials safely while that lightweight stack is in place.

## OAuth Secret Rotation Playbook
...
## Cost Guardrails (Secret-Focused)
...
## When to Revisit Storage

- Monitor the frequency of instance restarts. Because data lives in `/tmp/quorvium-data`, any restart erases boards.
- Once testing requires persistence across deploys or multiple instances, migrate back to the Cloud SQL plan captured in `docs/production-plan.md` and extend Terraform accordingly.
